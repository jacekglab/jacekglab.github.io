...
const getters = {
    ...
    currentTemp: state => {
        if (state.units === "imperial") {
            return {
                avg: parseInt(state.weather.current.temp_c * 1.8) + 32 + "\xB0F",
                min: parseInt(state.weather.forecast.forecastday[0].day.mintemp_c * 1.8) + 32 + "\xB0F",
                max: parseInt(state.weather.forecast.forecastday[0].day.maxtemp_c * 1.8) + 32 + "\xB0F"
            };
        } else {
            return {
    ...
    //store.getters.forecast.forecastday[$store.getters.activeDay].hour
    dashDetails: state => {
        if (state.units === "imperial") {
            return {
                humidity: state.weather.forecast.forecastday.map(forecastday => {
                    return forecastday.day.avghumidity + "%";
                }),
                precip: state.weather.forecast.forecastday.map(forecastday => {
                    return parseInt(forecastday.hour.reduce((acc, curr, index) => acc + curr.precip_mm, 0) / forecastday.hour.length * (1 / 25.4)) + " in";
                }),
                pressure: state.weather.forecast.forecastday.map(forecastday => {
                    return parseInt(forecastday.hour.reduce((acc, curr, index) => acc + curr.pressure_mb, 0) / forecastday.hour.length) + " mb";
                }),
                temperature: state.weather.forecast.forecastday.map(forecastday => {
                    return parseInt(forecastday.day.avgtemp_c * 1.8) + 32 + "\xB0F";
                }),
                uvindex: state.weather.forecast.forecastday.map(forecastday => {
                    return forecastday.day.uv;
                }),
                wind: state.weather.forecast.forecastday.map(forecastday => {
                    return parseInt(forecastday.hour.reduce((acc, curr, index) => acc + curr.wind_kph, 0) / forecastday.hour.length * 0.62) + " MPH";
                })
            };
        } else {
            return {
                ...
                precip: state.weather.forecast.forecastday.map(forecastday => {
                    return parseInt(forecastday.hour.reduce((acc, curr, index) => acc + curr.precip_mm, 0) / forecastday.hour.length) + " mm";
                }),
                pressure: state.weather.forecast.forecastday.map(forecastday => {
                    return parseInt(forecastday.hour.reduce((acc, curr, index) => acc + curr.pressure_mb, 0) / forecastday.hour.length) + " hPa";
                }),
                temperature: state.weather.forecast.forecastday.map(forecastday => {
                    return forecastday.day.avgtemp_c + "\xB0C";
                }),
                uvindex: state.weather.forecast.forecastday.map(forecastday => {
                    return forecastday.day.uv;
                }),
                wind: state.weather.forecast.forecastday.map(forecastday => {
                    return parseInt(forecastday.hour.reduce((acc, curr, index) => acc + curr.wind_kph, 0) / forecastday.hour.length) + " KPH";
                })
            };
        }
    }
...
//this.$store.getters.forecast.forecastday[$store.getters.activeDay].hour, "wind_kph"
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    }, [_c('offset'), _vm._v(" "), _c('div', {
      staticClass: "icon",
      class: _vm.mapIcon(item.day.condition.icon)
    }), _vm._v(" "), _c('p', [_vm._v(" " + _vm._s(_vm.mapDate(item.date, index)) + " ")]), _vm._v(" "), _c('offset')], 1) : _c('div', {
      staticClass: "active"
    }, [_c('offset'), _vm._v(" "), _c('div', {
      staticClass: "icon",
      class: _vm.mapIcon(item.day.condition.icon)
    }), _vm._v(" "), _c('p', [_vm._v(" " + _vm._s(_vm.mapDate(item.date, index)) + " ")]), _vm._v(" "), _c('offset')], 1)])
  }))])])
},staticRenderFns: []}
...
//# sourceMappingURL=app.4a23838bbf82496bf20f.js.map